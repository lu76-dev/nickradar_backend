...
app.get('/', (req, res) => {
  res.json({ message: 'nickradar API v6.3.0', status: 'running' });
});
...
app.listen(PORT, '0.0.0.0', async () => {
  console.log(`nickradar API v6.3.0 running on port ${PORT}`);
  try {
    await pool.query(`
    ...
    console.log('DB schema v6.3.0 ready');
  } catch (err) {
