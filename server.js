...
app.get('/api/events/:id/print-back', requireEventAdminAuth, async (req, res) => {
  try {
    ...
    const instrCell = `<div class="instr"><img class="instr-logo" src="https://app.nickradar.com/nr_logo.png" alt="" /><div class="instr-brand">nickradar</div><img class="instr-qr" src="${QR_URL}" alt="QR" /><div class="instr-steps"><div class="step-row"><div class="step"><span class="sn">1</span>open app.nickradar.com</div><div class="step"><span class="sn">2</span>enter your 4-digit code</div></div><div class="step-row"><div class="step"><span class="sn">3</span>stick it &mdash; stay visible</div><div class="step"><span class="sn">4</span>search nicknames &middot; connect</div></div></div></div>`;
    let sheetsHtml = '';
    for (let p = 0; p < pages; p++) {
      const pageStickers = stickers.rows.slice(p * perPage, (p + 1) * perPage);
      while (pageStickers.length < perPage) pageStickers.push(null);
