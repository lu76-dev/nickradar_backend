...
app.post('/api/events', requireEventAdminAuth, async (req, res) => {
  const { event_name, org_name, start_at, sticker_count, timezone } = req.body;
...
app.get('/', (req, res) => {
  res.json({ message: 'nickradar API v6.3.3', status: 'running' });
});
...
app.listen(PORT, '0.0.0.0', async () => {
  console.log(`nickradar API v6.3.3 running on port ${PORT}`);
  try {
    await pool.query(`
    ...
    console.log('DB schema v6.3.3 ready');
  } catch (err) {
